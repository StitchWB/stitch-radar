...
Spawns ``python -m stitch_radar`` (no host dependency) and walks the
JSON-RPC 2.0 line protocol: init → _migrate_db → ping → shutdown.
Radar commands depend on the live AiApiRadar backend, so the smoke test
stops at liveness.  Mirrors the starter test in the plugin template repo.
"""
